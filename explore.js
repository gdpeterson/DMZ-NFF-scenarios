console.log("[explore.js v10] loaded");
...
function renderAxes(){ if(AXIS_LABEL_NODES.length!==3) return; const L=I18N[LANG].axes; AXIS_LABEL_NODES[0].text(L[0]); AXIS_LABEL_NODES[1].text(L[1]); AXIS_LABEL_NODES[2].text(L[2]); }
function setLang(lang){ LANG=lang; localStorage.setItem("dmz-lang",LANG);
  document.getElementById("btn-en")?.classList.toggle("active", LANG==="en");
  document.getElementById("btn-kr")?.classList.toggle("active", LANG==="kr");
  renderAxes(); if(CURRENT_VISION) SHOW_CARD(CURRENT_VISION);
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  svg.append("path")
    ...
    .style("filter","drop-shadow(0 6px 14px rgba(15,23,42,.08))").style("pointer-events","all");
  ...
  setLang(LANG); renderAxes();

  function baryToXY(nff){ const a=nff.a??nff.nature_for_nature??0, b=nff.b??nff.nature_for_society??0, c=nff.c??nff.nature_as_culture??0;
    const s=a+b+c||1, aa=a/s, bb=b/s, cc=c/s; return {x:aa*A.x+bb*B.x+cc*C.x+origin.x, y:aa*A.y+bb*B.y+cc*C.y+origin.y}; }
  const clamp=(v,min,max)=>Math.max(min,Math.min(max,v));

  // Load data (visions required; others optional)
  let visions=[], seeds=[], stories=[];
  try{ const r=await fetch("data/visions.json"); if(!r.ok) throw new Error(`visions.json ${r.status}`); visions=await r.json(); }catch(e){
    document.getElementById("scenario-card").innerHTML=`<strong>Load error:</strong> ${e.message}`; return; }
  ...
  function nearestVision(x,y,list){ let best=null, bestD=Infinity;
    for(const v of list){ const p=baryToXY(v.nff), d=(p.x-x)**2+(p.y-y)**2; if(d<bestD){bestD=d; best=v;} } return best; }
  ...
  visions.forEach(v=>{
    ...
    gPoints.append("circle").attr("cx",p.x).attr("cy",p.y).attr("r",8).attr("fill",color).attr("stroke","#fff").attr("stroke-width",2)
      ...
      .on("mouseleave",function(){d3.select(this).transition().attr("r",8);}).on("click",()=>selectVision(v,p));
    ...
    gTags.append("text").attr("x",lx).attr("y",ly).attr("class","scenario-tag").attr("fill",color)
      .attr("text-anchor",anchor).text(v.title_en||v.id).style("cursor","pointer").on("click",()=>selectVision(v,p));
  });
  ...
  visions.forEach((v,i)=>{ const y=22+i*20, color=palette(v.title_key||v.title_en||v.id);
